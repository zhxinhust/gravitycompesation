...
private:
    ...
    Vector4d w0, w1, w2, w3, w4, w5;    // 各轴方向
    Vector4d p0, p1, p2, p3, p4, p5;    // 各轴上的点

    Matrix4d frame_base_left, frame_base_right; // support到各臂基座标的变换矩阵
    Matrix4d gst;   // 在各关节值为0时，末端坐标值

    int getUr3Jac(JntArray q_in, Matrix4d frame_base, Jacobian &jac);
...
public:
    ...
    int IK_vel_left (const JntArray &q_in, const KDL::Twist &v_in, JntArray &qdot_out);

    int IK_vel_right (const JntArray &q_in, const KDL::Twist &v_in, JntArray &qdot_out);
    ...
    VectorXd Get_singularvalues_left(JntArray q_in);

    VectorXd Get_singularvalues_right(JntArray q_in);
...
void getVelFromCenter(Frame frame_center, float r, KDL::Twist vel_center, KDL::Twist &vel_left, KDL::Twist &vel_right);
